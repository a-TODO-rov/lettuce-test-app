...
/**
 * Listens to Lettuce connection events via the EventBus and records metrics.
 * <p>
 * Uses callback-based subscription: {@code eventBus.subscribe(Consumer)}. No Reactor dependency required.
 * <p>
 * This listener is ONLY loaded when using the reactor-optional Lettuce fork, which provides
 * the callback-based subscribe method. For official Lettuce, use ReactiveLettuceEventListener.
 */
@Component
@ConditionalOnExpression("#{T(io.lettuce.test.metrics.LettuceEventListener).hasCallbackSubscribe()}")
public class LettuceEventListener {

    /**
     * Check if EventBus has the callback-based subscribe(Consumer) method.
     * This method only exists in the reactor-optional fork.
     */
    public static boolean hasCallbackSubscribe() {
        try {
            Method method = EventBus.class.getMethod("subscribe", Consumer.class);
            return method != null;
    ...
    protected Closeable subscription;
    ...
    public LettuceEventListener(ClientResources clientResources, MeterRegistry meterRegistry) {
        ...
        log.info("LettuceEventListener initialized (callback-based)");
    }

    @PostConstruct
    public void startListening() {
        subscription = eventBus.subscribe(this::handleEvent);
        log.info("Started listening to Lettuce events (callback-based)");
    }
